...
export default function SenderParcels() {
  ...
  const searchSortUI = (
    <div className="w-full max-w-7xl mx-auto flex items-center justify-between gap-3 mt-10 mb-5">
      <h1>My Parcels</h1>
      <DashboardSearch
  ...
  if (ParcelData?.data?.result?.length === 0) {
    return (
      <>
        {searchSortUI}
        <div className="max-w-7xl w-full mx-auto bg-background overflow-hidden rounded-md border flex items-center justify-center">
          ...
          <TableBody>
            {ParcelData?.data?.data?.map((parcel: IParcel) => (
              <TableRow key={parcel._id}>
                ...
                <TableCell className="py-2">{parcel?.fee} Tk</TableCell>
                <TableCell className="py-2">
